# 👋 Olá, eu sou Sergio Araujo

🎯 Profissional focado em Tecnologia, Dados e Gestão Ágil  
📍 Estou locado em Mauá, na grande São Paulo,Brasil  
💼 Interesse em Desenvolvimento, Automação, Produtos e Projetos  

## 📫 Contato

- WhatsApp: +55 11 981644800
- LinkedIn: https://linkedin.com/in/sergioaraujo
- Email: sergio.a.araujo1971@gmail

## 🚀 Sobre Mim

Atuo com desenvolvimento e automação de processos, criando soluções que otimizam tarefas, melhoram a qualidade dos dados e apoiam a tomada de decisão.

Tenho experiência com:
- 📊 Automação de planilhas Excel
- 📁 Manipulação de bases de dados em arquivos texto
- 📈 Criação de dashboards de negócios
- 🔄 Organização e versionamento de código com Git e GitHub
- ⚙️ Metodologias Ágeis (Scrum e Kanban)

Busco sempre desenvolver soluções eficientes, organizadas e alinhadas às necessidades do negócio, especialmente no mercado financeiro.

---

## 🛠️ Tecnologias e Ferramentas

- Python
- Pandas
- PyAutoGui
- Git & GitHub
- VS Code
- Jupyter Notebook
- Excel
- Metodologias Ágeis (Scrum / Kanban)
- Gestão de Projetos
- Linguagem SQL
- Java Script
- 

---

## 📂 Projetos em Destaque

Aqui você encontrará projetos relacionados a:

- ✅ Automação de tarefas operacionais
- ✅ Tratamento e análise de dados
- ✅ Geração de relatórios e dashboards
- ✅ Scripts para otimização de processos

*(Em constante atualização)*

---

## 📊 Objetivo

Consolidar experiência em desenvolvimento e gestão ágil, contribuindo para projetos que gerem valor real ao negócio e aprimorem processos através da tecnologia.

---



---

⭐ Se algum projeto for útil para você, fique à vontade para deixar uma estrela!

